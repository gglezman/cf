...
class AccountEditWin:
    ...
    *Architecture of this module...*
    ...
    (The next_date (in the occurrence sequence) and the regularity (e.g. monthly,
    yearly) are extracted from the occurrence when needed for display).
    ...
    *Args*
        ...
        title (str): Title to display on the popup window
    ...
    *column_descriptor details:*
    ...
    def add_filter_widgets(self, filter_frame):
        """Add the filter widgets(combo buttons) to the filter frame.
    ...
    def create_account_widgets(self, frame, column_descriptor):
        ...
        in the list represents an item in the account list. That entry is
    ...
    def bind_account_widgets(self, account_widgets, column_descriptor):
        ...
        This method assumes id's in records correspond to row in the
        account_widget list.
        """
    ...
    def bind_widget_row(self, rec_id, widget_row, column_descriptor):
        """This method will bind each widget in a single_row to an action.

        This method binds the "action menu" to all widgets in the row.
        It also binds functions to datepickers and check buttons widgets.
        """

        for col, column in enumerate(column_descriptor):
            if column["type"] == "date":
                if column["content"] == "standard":
                    widget_row[col].configure(
                        command=lambda colm=col, id_=rec_id: self.open_datepicker(colm, id_))
                elif column["content"] == "latest":
                    widget_row[col].configure(
                        command=lambda colm=col, id_=rec_id: self.open_occurrence_win(colm, id_))
                ...
                if column["content"] == "call":
                    widget_row[col].configure(
                        command=lambda colm=col, id_=rec_id: self.open_bond_call(colm, id_))
    ...
    def add_account_widgets(self):
        ...
        are not necessarily sequential. Each record has an id
    ...
    def menu_on_item(self, id_):
        ...
        if not self.actions_menu:
            ...
            # instrument have the latest data provided by the user (e.g. user changes
    ...
    def close_subordinate_windows(self):
        ...
        for key, datepicker_obj in self.datepicker_windows.values():
            datepicker_obj.kill_win()

        for key, occurrence_obj in self.occurrence_windows.values():
    ...
    def close_and_update_win(self):
        ...
        account edit window and write the results to the database.
        ...
        # restart to accommodate all the changes
        self.parent.restart()
    ...
    def new_instrument(self):
        ...
        if self.instrument_type == 'ca':
            messagebox.showerror("Create Error",
                                 "The cash account can not be directly created."
                                 "Creating an Account will create the associated"
                                 "cash account!")
            return
    ...
    def new_instrument_closed(self, new_instrument_obj, new_rec):
        ...
        example, a new account will generate a new cash account)
        """
    ...
    def open_datepicker(self, col, id_):
        """Open a date picker so the user can select an alternate date"""
    ...
    def OccurrenceWin_return(self, occurrence_spec, token):
        ...
        """User has closed an Occurrence window."""
...
class ActionsOnInstrument:
    ...
    def __init__(self, account_edit, parent, account_edit_win,
                 data_source, id_, instrument_type):
        self.account_edit = account_edit  # AccEditWin Class
        self.parent = parent  # GUI
        self.parent_win = account_edit_win  # top level Edit window
        ...
        if platform.system() != 'Linux':
            ...
            # have to close it
            self.action_win.transient(account_edit_win)

        # remove the window dressing
        self.action_win.overrideredirect(True)
        # the following is an ex11 hack and only works for Linux
        # self.action_win.wm_attributes('-type', 'splash')
        util.center_popup(self.action_win, account_edit_win)
    ...
    def delete_item(self):
        if self.instrument_type == 'ca':
            ...
            messagebox.showerror("Delete Error",
                                 ...
                                 "Deleting the Account will delete the associated cash account!")
            return
    ...
    def get_inflated_amounts(self):
        ...
        if dates:
            ...
            amounts = self.parent.get_inflated_amounts(transfer_spec['amount'], transfer_spec['inflation'],
                                                       occ.get_dates())
...
class BondCall:
    def __init__(self, parent, parent_win, bond_record, col, rec_id):
        ...
        self.col = col  # col num - used for checkbutton maintenance
        self.rec_id = rec_id
    ...
    def bind_widgets(self, widget_row, column_descriptor):
        ...
        for col, column in enumerate(column_descriptor):
            if column["type"] == "date":
                if column["content"] == "standard":
                    widget_row[col].configure(
                        command=lambda colm=col: self.open_datepicker(colm))
                elif column["content"] == "latest" or \
                        column["content"] == "regularity":
                    widget_row[col].configure(
                        command=lambda colm=col: self.open_occurrence_win(colm))
                else:
                    raise TypeError("Unknown date content: {}".format(
                        column["content"]))
